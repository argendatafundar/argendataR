...
argendata_root_dir <- function() {
  ...
  if (length(filetemp) == 1) { 
    ...
    readr::write_rds(argendata_root_dir, file = tempfile("argendata_root_dir"))
...
bbdd_dir <- function() {
  ...
  if (length(filetemp) == 1) { 
    ...
    readr::write_rds(bbdd_dir, file = tempfile(pattern = "bbdd_dir", fileext = ".rds"))
...
fuentes_dir <- function(){
  ...
  if (length(filetemp) == 1) { 
    ...
    readr::write_rds(fuentes_dir, file = tempfile(pattern = "fuentes_dir", fileext = ".rds"))
...
fuentes_raw_dir <- function() {
  ...
  if (length(filetemp) == 1) { 
    ...
    readr::write_rds(fuentes_raw_dir, file = tempfile(pattern = "fuentes_raw_dir",
                                                      fileext = ".rds"))
...
fuentes_clean_dir <- function() {
  ...
  if (length(filetemp) == 1) { 
    ...
    readr::write_rds(fuentes_clean_dir, tempfile(pattern = "fuentes_clean_dir",
                                                 fileext = ".rds"))
